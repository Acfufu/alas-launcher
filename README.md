...
macOS 上还带一个菜单栏图标：不用打开窗口就能查看 ALAS 任务列表、启停后端。先看下方的[菜单栏速览动图](#菜单栏速览macos)，再对照[启动流程动图](#启动流程)与[真实界面截图](#界面截图)，然后选择你的[下载平台](#下载与平台)。

## 菜单栏速览（macOS）

![ALAS 菜单栏动图：点击菜单栏舰船图标弹出原生菜单，显示后端运行状态与停止/启动开关；随后展示 Web UI 调度器按「运行中 / 队列中 / 等待中」分组的真实 ALAS 任务列表（含下次调度时间）与主窗口](assets/readme/tray-zh.gif)

- **任务列表实时可见**：与 Web UI 调度器一致，按 `运行中 / 队列中 / 等待中` 分组展示真实 ALAS 任务与下次调度时间（每组最多 3 条，保持菜单紧凑；10 秒自动刷新，也可手动刷新）。
- **后端一键启停**：不用退出应用即可停止或重新启动 `gui.py`；窗口会随状态切换显示「后端已停止」提示页或恢复 Web UI。
...
2. **macOS 菜单栏速览**：菜单栏图标直接查看 ALAS 任务列表（运行中/队列中/等待中分组、下次调度时间），并可启停后端而无需退出应用。
